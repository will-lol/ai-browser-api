...
export type ProviderPromptMessage = LanguageModelV3CallOptions["prompt"][number]
export type ProviderToolSpec = NonNullable<LanguageModelV3CallOptions["tools"]>[number]
...
export type ProviderUserPart = Extract<ProviderPromptMessage, { role: "user" }>["content"][number]
export type ProviderAssistantPart = Extract<ProviderPromptMessage, { role: "assistant" }>["content"][number]
export type ProviderToolPart = Extract<ProviderPromptMessage, { role: "tool" }>["content"][number]
export type ProviderToolResultOutput = Extract<ProviderAssistantPart, { type: "tool-result" }>["output"]
...
export type RuntimeToolResultOutput = Extract<RuntimeAssistantPart, { type: "tool-result" }>["output"]

export function assertNever(value: never, message = "Unexpected bridge codec variant"): never {
...
export function encodeWireDate(value: Date | undefined): RuntimeWireDate | undefined {
  if (!value) {
    return undefined
  }
...
export function decodeWireDate(value: RuntimeWireDate | undefined): Date | undefined {
  if (!value) {
    return undefined
  }
...
export function toContractJsonObject(value: JSONObject): { readonly [key: string]: JsonValue } {
...
export function toRuntimeUsage(usage: LanguageModelV3GenerateResult["usage"]): RuntimeUsage {
  return {
    inputTokens: {
...
export function fromRuntimeUsage(usage: RuntimeUsage): LanguageModelV3GenerateResult["usage"] {
  return {
    inputTokens: {
      total: usage.inputTokens.total,
      noCache: usage.inputTokens.noCache,
...
export function toRuntimeWarnings(
  warnings: LanguageModelV3GenerateResult["warnings"] | ReadonlyArray<SharedV3Warning>,
