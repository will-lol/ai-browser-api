...
type ProviderToolSpec = NonNullable<LanguageModelV3CallOptions["tools"]>[number]

type ContractUserMessage = Extract<ContractPromptMessage, { role: "user" }>
type ContractAssistantMessage = Extract<ContractPromptMessage, { role: "assistant" }>
...
type ContractToolResultOutput = Extract<ContractAssistantMessage["content"][number], { type: "tool-result" }>["output"]
...
export interface BridgeClientApi {
  readonly listModels: Effect.Effect<ReadonlyArray<BridgeModelSummary>, RuntimeRpcError>
  getModel: (modelId: string) => Effect.Effect<LanguageModelV3, RuntimeRpcError>
  readonly getState: Effect.Effect<BridgeStateResponse, RuntimeRpcError>
  requestPermission: (payload?: BridgePermissionRequest) => Effect.Effect<BridgePermissionResult, RuntimeRpcError>
  readonly destroy: Effect.Effect<void, never>
}

export class BridgeClient extends Context.Tag("@llm-bridge/client/BridgeClient")<
  BridgeClient,
  BridgeClientApi
>() {}
...
function log(debug: boolean, logger: typeof console.info, event: string, meta?: string) {
  if (!debug) return
...
function isWireUrl(value: unknown): value is RuntimeWireUrl {
  return isRecord(value) && value.__llmBridgeWireType === "url" && typeof value.href === "string"
}
...
function encodeDataContent(value: string | URL | Uint8Array): string | RuntimeWireUrl | RuntimeWireUint8Array {
...
function decodeGeneratedBinaryData(value: RuntimeWireUint8Array): Uint8Array {
  ...
  throw new RuntimeValidationError({
    message: "Failed to decode generated binary data from runtime wire transport",
  })
}
...
function encodeToolResultOutput(output: ProviderToolResultOutput): ContractToolResultOutput {
  switch (output.type) {
    case "text":
      return {
        type: "text",
...
function encodeUserPart(part: ProviderUserPart): ContractUserMessage["content"][number] {
  if (part.type === "text") {
    return {
      type: "text",
      text: part.text,
...
function encodeAssistantPart(part: ProviderAssistantPart): ContractAssistantMessage["content"][number] {
  switch (part.type) {
    case "text":
      return {
        type: "text",
...
function encodeToolPart(part: ProviderToolPart): ContractToolMessage["content"][number] {
...
function encodePromptMessage(message: ProviderPromptMessage): ContractPromptMessage {
  switch (message.role) {
    case "system":
      return {
        role: "system",
...
function encodeResponseFormat(
  ...
  return {
    type: "json",
    schema: responseFormat.schema ? encodeRuntimeWireValue(responseFormat.schema) : undefined,
    name: responseFormat.name,
    description: responseFormat.description,
  }
}

function encodeCallOptions(options: LanguageModelV3CallOptions): RuntimeModelCallOptions {
  return {
  ...
  switch (part.type) {
    ...
    case "file":
      return {
        type: "file",
        mediaType: part.mediaType,
        data: typeof part.data === "string" ? part.data : decodeGeneratedBinaryData(part.data),
        providerMetadata: toProviderMetadata(part.providerMetadata),
      }
...
function decodeGenerateResponse(response: RuntimeGenerateResponse): LanguageModelV3GenerateResult {
  return {
    ...
    usage: {
      ...
      raw: response.usage.raw ? toProviderJsonObject(response.usage.raw) : undefined,
    },
    providerMetadata: toProviderMetadata(response.providerMetadata),
    request: response.request
      ? {
          body: response.request.body === undefined
            ? undefined
    ...
    response: response.response
      ? {
          ...
          headers: response.response.headers ? { ...response.response.headers } : undefined,
          body: response.response.body === undefined
            ? undefined
            : decodeRuntimeWireValue(response.response.body),
        }
      : undefined,
    warnings: response.warnings.map((warning) => ({ ...warning })) as Array<SharedV3Warning>,
  }
}
...
function decodeStreamPart(part: RuntimeStreamPart): LanguageModelV3StreamPart {
  switch (part.type) {
    ...
    case "stream-start":
      return {
        type: "stream-start",
        warnings: part.warnings.map((warning) => ({ ...warning })) as Array<SharedV3Warning>,
      }
    ...
    case "finish":
      return {
        ...
        usage: {
          ...
          raw: part.usage.raw ? toProviderJsonObject(part.usage.raw) : undefined,
        },
        finishReason: {
...
function createConnection(options: BridgeClientOptions): Effect.Effect<BridgeConnection, RuntimeRpcError> {
  ...
  return Effect.tryPromise({
    try: async () => {
      ...
      const onMessage = (event: MessageEvent<FromServer<PageBridgeRpc>>) => {
        void Effect.runPromise(write(event.data)).catch((error) => {
          log(debug, logger, "rpc.writeError", error instanceof Error ? error.message : String(error))
        })
      }
    ...
    Effect.gen(function*() {
      ...
      const ensureConnection = Effect.tryPromise({
        try: async () => {
          ...
          if (!connectionPromise) {
            connectionPromise = Effect.runPromise(createConnection(options)).then((value) => {
              connection = value
              return value
            })
          }
      ...
      const abortRequest = (requestId: string) =>
        ensureConnection.pipe(
          Effect.flatMap((current) => current.client.abort({ requestId })),
          Effect.asVoid,
        )
      ...
      const createLanguageModel = (
        ...
        async doGenerate(options) {
          ...
          const onAbort = () => {
            void Effect.runPromise(abortRequest(requestId)).catch(() => undefined)
          }
      ...
      const getState = ensureConnection.pipe(
        Effect.flatMap((current) => current.client.getState({})),
      )
...
export function withBridgeClient<R, E, A>(
  ...
  return effect.pipe(
    Effect.provide(BridgeClientLive(options)),
  )
}
