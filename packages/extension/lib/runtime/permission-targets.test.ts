...
const {
  resolveTrustedPermissionTarget,
} = await import("./permission-targets")
...
describe("permission target resolution", () => {
  it("resolves connected models as trusted permission targets", async () => {
    ...
    await expect(resolveTrustedPermissionTarget(CONNECTED_MODEL_ID)).resolves.toEqual({
      status: "resolved",
      target: {
  ...
  it("marks missing models as missing", async () => {
    await expect(resolveTrustedPermissionTarget(MISSING_MODEL_ID)).resolves.toEqual({
      status: "missing",
  ...
  it("marks disconnected providers as disconnected", async () => {
    ...
    await expect(resolveTrustedPermissionTarget(DISCONNECTED_MODEL_ID)).resolves.toEqual({
      status: "disconnected",
