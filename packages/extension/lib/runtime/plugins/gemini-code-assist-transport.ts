...
function transformRequestPayload(input: unknown, projectId: string, model: string) {
  ...
  if (typeof body.project === "string" && body.request && typeof body.request === "object") {
    return {
      ...body,
      project: projectId,
      model,
...
export async function rewriteGeminiCodeAssistRequest(
  ...
  if (!authorization) {
    throw new Error("Gemini OAuth bearer token is missing from request headers.")
  }
  ...
  const nextBody = (() => {
    ...
    const wrapped = transformRequestPayload(parsedBody, options.projectId, parsed.effectiveModel)
...
export function transformGeminiCodeAssistSSELine(line: string): string {
  ...
  try {
    ...
    if (typeof parsed.traceId === "string") {
      ...
      if (typeof responseRecord.responseId !== "string" || !responseRecord.responseId) {
        ...
        reader
          ...
          .then(({ done, value }) => {
            if (done) {
              ...
              if (buffer.length > 0) {
                controller.enqueue(encoder.encode(transformGeminiCodeAssistSSELine(buffer)))
              }
              controller.close()
              return
            }
...
export async function normalizeGeminiCodeAssistResponse(
  ...
  if (metadata.streaming && contentType.includes("text/event-stream") && response.body) {
...
export function createGeminiCodeAssistFetch(options: GeminiCodeAssistFetchOptions): typeof fetch {
  return createTransportFetchPipeline({
    fetchFn: options.fetchFn,
    maxAttempts: options.maxAttempts,
    baseRetryDelayMs: options.baseRetryDelayMs,
    rewriteRequest: (input, init) => rewriteGeminiCodeAssistRequest(input, init, { projectId: options.projectId }),
