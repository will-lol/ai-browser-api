...
function createOnBeforeRequestHarness(input?: {
  throwOnAdd?: Error
}) {
...
function buildOptions(input?: {
  ...
  return {
    ...
    matchesUrl: (url: string) => url.startsWith("http://localhost:1455/auth/callback"),
...
describe("waitForOAuthCallback", () => {
  it("resolves on main_frame callback, emits hooks, and cleans listener once", async () => {
    ...
    const wait = waitForOAuthCallback(buildOptions({
  ...
  it("throws provider-defined unsupported error when webRequest is unavailable", async () => {
    await assert.rejects(
      () => waitForOAuthCallback(buildOptions({
