...
type EnsureConnection<Rpcs extends Rpc.Any> = () => Promise<RuntimeConnection<Rpcs>>
type RuntimeRpcTag<Rpcs extends Rpc.Any> = Rpcs["_tag"]
type RuntimeRpcMethod<Rpcs extends Rpc.Any, Tag extends RuntimeRpcTag<Rpcs>> = Extract<Rpcs, { readonly _tag: Tag }>
...
type RuntimeRpcFacadeMethod<Current extends Rpc.Any> =
  Rpc.SuccessSchema<Current> extends RpcSchema.Stream<infer _A, infer _E>
    ? (input: Rpc.PayloadConstructor<Current>) => AsyncIterable<Rpc.SuccessChunk<Current>>
...
export function createRuntimeRpcFacade<Rpcs extends Rpc.Any>(input: {
  ...
  for (const rpcDefinition of input.rpcGroup.requests.values()) {
    ...
    if (RpcSchema.isStreamSchema(rpc.successSchema)) {
      const method = ((requestInput: Rpc.PayloadConstructor<RuntimeRpcMethod<Rpcs, typeof tag>>) => ({
        async *[Symbol.asyncIterator]() {
          ...
          ) => Stream.Stream<Rpc.SuccessChunk<RuntimeRpcMethod<Rpcs, typeof tag>>, never, never>
    ...
    const method = (async (requestInput: Rpc.PayloadConstructor<RuntimeRpcMethod<Rpcs, typeof tag>>) => {
      const { client } = await input.ensureConnection()
      const request = (client as RuntimeRpcClientRecord)[tag] as (
        payload: Rpc.PayloadConstructor<RuntimeRpcMethod<Rpcs, typeof tag>>,
      ) => Effect.Effect<Rpc.Success<RuntimeRpcMethod<Rpcs, typeof tag>>, never, never>
