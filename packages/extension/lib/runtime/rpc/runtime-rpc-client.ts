...
export function createRuntimeAdminRpcClient(input: {
  ...
  return {
    ...
    listProviders: (payload: Parameters<RuntimeRpcClientConnection<RuntimeRpc>["listProviders"]>[0]) =>
      Effect.flatMap(input.ensureClient, (client) => client.listProviders(payload)),
    listConnectedModels: (payload: Parameters<RuntimeRpcClientConnection<RuntimeRpc>["listConnectedModels"]>[0]) =>
      Effect.flatMap(input.ensureClient, (client) => client.listConnectedModels(payload)),
    listPermissions: (payload: Parameters<RuntimeRpcClientConnection<RuntimeRpc>["listPermissions"]>[0]) =>
      Effect.flatMap(input.ensureClient, (client) => client.listPermissions(payload)),
    openProviderAuthWindow: (payload: Parameters<RuntimeRpcClientConnection<RuntimeRpc>["openProviderAuthWindow"]>[0]) =>
      Effect.flatMap(input.ensureClient, (client) => client.openProviderAuthWindow(payload)),
    getProviderAuthFlow: (payload: Parameters<RuntimeRpcClientConnection<RuntimeRpc>["getProviderAuthFlow"]>[0]) =>
      Effect.flatMap(input.ensureClient, (client) => client.getProviderAuthFlow(payload)),
    startProviderAuthFlow: (payload: Parameters<RuntimeRpcClientConnection<RuntimeRpc>["startProviderAuthFlow"]>[0]) =>
      Effect.flatMap(input.ensureClient, (client) => client.startProviderAuthFlow(payload)),
    cancelProviderAuthFlow: (payload: Parameters<RuntimeRpcClientConnection<RuntimeRpc>["cancelProviderAuthFlow"]>[0]) =>
      Effect.flatMap(input.ensureClient, (client) => client.cancelProviderAuthFlow(payload)),
    disconnectProvider: (payload: Parameters<RuntimeRpcClientConnection<RuntimeRpc>["disconnectProvider"]>[0]) =>
    ...
    setOriginEnabled: (payload: Parameters<RuntimeRpcClientConnection<RuntimeRpc>["setOriginEnabled"]>[0]) =>
      Effect.flatMap(input.ensureClient, (client) => client.setOriginEnabled(payload)),
    setModelPermission: (payload: Parameters<RuntimeRpcClientConnection<RuntimeRpc>["setModelPermission"]>[0]) =>
      Effect.flatMap(input.ensureClient, (client) => client.setModelPermission(payload)),
    resolvePermissionRequest: (payload: Parameters<RuntimeRpcClientConnection<RuntimeRpc>["resolvePermissionRequest"]>[0]) =>
      Effect.flatMap(input.ensureClient, (client) => client.resolvePermissionRequest(payload)),
    dismissPermissionRequest: (payload: Parameters<RuntimeRpcClientConnection<RuntimeRpc>["dismissPermissionRequest"]>[0]) =>
      Effect.flatMap(input.ensureClient, (client) => client.dismissPermissionRequest(payload)),
  };
}
