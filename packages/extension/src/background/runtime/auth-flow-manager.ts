...
import type { RuntimeAuthMethod } from "@/background/runtime/adapters/types";
import {
  listProviderAuthMethods,
  startProviderAuth,
} from "@/background/runtime/provider-auth";
...
class AuthFlowManager {
  ...
  async getProviderAuthFlow(
    ...
    if (
      flow.expiresAt <= this.currentTimestamp() &&
      !isTerminalStatus(flow.status)
    ) {
  ...
  async openProviderAuthWindow(
    providerID: string,
  ): Promise<OpenProviderAuthWindowResult> {
