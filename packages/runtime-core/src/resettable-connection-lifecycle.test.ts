  ...
  it("retries after a failed connect attempt", async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const lifecycle = yield* makeResettableConnectionLifecycle<number, string>({
          create: () =>
  ...
  it("destroy invalidates waiters and closes stale late success", async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const waitingFiber = yield* Effect.fork(Effect.either(lifecycle.ensure))
  ...
  it("destroyIfCurrent no-ops for stale tokens", async () => {
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const lifecycle = yield* makeResettableConnectionLifecycle<number, string, "disconnect">({
          create: (token) => Effect.succeed(token),
          close: (value, reason) =>
            Ref.update(closeReasons, (current) => [...current, `${reason}:${value}`]),
          invalidatedError: () => "invalidated",
        })
